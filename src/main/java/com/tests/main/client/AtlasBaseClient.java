...
public abstract class AtlasBaseClient {
    ...
    void initializeState(Configuration configuration, String[] baseUrls, UserGroupInformation ugi, String doAsUser) {
        ...
        if (StringUtils.isEmpty(basicAuthUser) && StringUtils.isEmpty(basicAuthPassword)) {
            final BearerAuthFilter authFilter = new BearerAuthFilter();
            client.addFilter(authFilter);
            LOG.info("Added BearerAuthFilter");
        }
